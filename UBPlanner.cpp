...
bool UBPlanner::decompose() {
    ...
    for (int j = 0; j < ystep; j++) {//for each step in the y direction
        for (int i = 0; i < xstep; i++) {//for each step in the x direction
            ...
            if (evaluate(cell)) {//if a cell passes eval, it is a visitable cell
...
bool UBPlanner::evaluate(const QVector<QPointF>& cell) {
    for (int i = 0; i < cell.size() - 1; i++) {
        ...
        foreach (QPolygonF area, m_areas) {//for all areas, outer and inner (obstacle)
...
//        cplex.exportModel("div.lp"); --Removed by Dr. Modares
        ...
        for (int a = 0; a < m_agents.size(); a++) {
            for (int i = 0; i < m_nodes.size(); i++) {
                if (cplex.getValue(x_agent_node[a][i])) {//if an agent takes a path
...
bool UBPlanner::planAgent(quint32 agent) {
    ...
    for (int i = 0; i < m_agent_paths[agent].size(); i++) {//for each agent
