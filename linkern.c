        ...
        CCrandstate *rstate)
{
    ...
    best = cycle_length(ncount, ctemp, D);
    ...
    while (round < quitcount) {
        ...
        if (IMPROVE_SWITCH == -1 || round < IMPROVE_SWITCH) {
            CClinkern_flipper_cycle(&F, ctemp);
            tval = cycle_length(ncount, ctemp, D);
            ...
            CClinkern_flipper_cycle(&F, ctemp);
            delta = cycle_length(ncount, ctemp, D) - tval;
        ...
        CCptrworld *edgelook_world)
{
    ...
    while (1) {
        ...
        CClinkern_flipper_cycle(F, ctemp);
        delta = tval - cycle_length(ncount, ctemp, D);
